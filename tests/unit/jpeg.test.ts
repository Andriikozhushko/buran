...
describe('JPEG cleaner', () => {
  ...
  it('preserves restart markers inside compressed image data', () => {
    const jpegWithRestartMarkers = Uint8Array.from([
      0xff, 0xd8,
      0xff, 0xe1, 0x00, 0x08, 0x45, 0x78, 0x69, 0x66, 0x00, 0x00,
      0xff, 0xdb, 0x00, 0x04, 0x00, 0x00,
      0xff, 0xda, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00,
      0x11, 0xff, 0xd0, 0x22, 0xff, 0xd1, 0x33, 0xff, 0xd9,
    ]).buffer;

    const clean = new Uint8Array(cleanJpeg(jpegWithRestartMarkers));

    expect(clean).toContain(0xff);
    expect(Array.from(clean)).toEqual(expect.arrayContaining([0xff, 0xd0, 0xff, 0xd1]));
    expect(Array.from(clean.slice(-2))).toEqual([0xff, 0xd9]);
    expect(Array.from(clean)).not.toEqual(expect.arrayContaining([0xff, 0xe1]));
  });
});
